...
const MapView: React.FC<MapViewProps> = ({ map, playerPos, playerDir, isMoving }) => {
    ...
    useEffect(() => {
        ...
        if (!tilesetRef.current) {
            tilesetRef.current = new Image();
        }
        if (!playerSpriteRef.current) {
            playerSpriteRef.current = new Image();
        }
        ...
        const checkLoaded = () => {
            if (tileset.complete && playerSprite.complete) {
        ...
        // playerSprite.onerror = (e) => {
        //     log(`Player ERROR: ${playerSprite.src}`);
        //     console.error("Player failed", e);
        // };

        tileset.src = '/assets/math-rpg/tilesets/world.png';
        playerSprite.src = '/assets/math-rpg/characters/player.png';
    ...
    const drawTile = (ctx: CanvasRenderingContext2D, tileId: number, x: number, y: number) => {
        ...
        if (tilesetRef.current && tilesetRef.current.complete && tilesetRef.current.naturalWidth > 0) {
